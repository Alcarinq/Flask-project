...
def follow(username):
    ...
    db.session.commit()
    flash(f'Zaczynasz obserwować użytkownika {username}')
    return redirect(url_for('.user', username=username))
...
def unfollow(username):
    ...
    if not current_user.is_following(user):
        flash('Nie obserwujesz tego użytkownika')
        return redirect(url_for('.user', username=username))
    current_user.unfollow(user)
    db.session.commit()
    flash(f"Przestajesz obserwować użytkownika {username}")
    return redirect(url_for('.user', username=username))
...
def followers(username):
    ...
    return render_template('followers.html', user=user, title="Obserwujący użytkownika", endpoint='.followers',
...
def followed_by(username):
    ...
    return render_template('followers.html', user=user, title="Obserwowany przez użytkownika", endpoint='.followed_by',
                           pagination=pagination, follows=follows)
